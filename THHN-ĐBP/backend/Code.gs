...
const CFG = {
  ...
  SHEET_AI_LOGS: "AI_LOGS",
    // ✅ GROUPS / ASSIGNMENTS (NEW)
  SHEET_GROUPS: "GROUPS",
  SHEET_GROUP_MEMBERS: "GROUP_MEMBERS",
...
function _cors_(output) {
  // Google Apps Script TextOutput/HtmlOutput KHÔNG hỗ trợ setHeader().
  // Nếu gọi trực tiếp sẽ gây lỗi runtime: "TypeError: output.setHeader is not a function".
  // => chỉ set header khi runtime có hỗ trợ, còn lại trả nguyên output.
  try {
...
function json_(obj) {
  // ✅ luôn đính CORS cho mọi output JSON (phòng trường hợp handler legacy còn dùng json_)
  return _cors_(
    ContentService
...
function handleRequest_(body) {
  ...
  ensureSheets_();
    // ✅ đảm bảo headers cho GROUPS / MEMBERS / ASSIGNMENTS
    GA_boot_();
...
function getPilot_() {
  ...
  return {
    ...
    // ✅ mặc định KHÔNG khoá pilot để Admin có thể tạo thêm lớp
    // (nếu muốn khoá đúng pilot 5A1 như trước thì set PILOT_LOCK="1")
    lock: (p.getProperty("PILOT_LOCK") || "0").trim(),
...
// SSOT normalize string (frontend gọi API, backend cần helper này)
function norm_(v) {
...
/**
 * Pilot lock (SSOT):
 * - Trước đây khoá cứng chỉ cho phép 1 lớp PILOT_CLASS_ID (gây không tạo thêm lớp).
 * - Từ bản này: nếu lớp đã tồn tại trong CLASSES thì cho phép (không phá luồng hiện có).
 * - Vẫn giữ thông báo pilot nếu lớp chưa tồn tại.
 */
function enforcePilotClass_(classId, yearOpt) {
  const pilot = getPilot_();
  const lock = pilot.lock === "1";
  const year = norm_(yearOpt) || norm_(pilot.year) || getActiveYear_();
  const cid = normalizeClassId_(classId || pilot.classId);
  if (!lock) return cid;

  // ✅ nếu lớp đã có trong CLASSES => cho phép dùng (để Admin mở rộng hệ thống)
  if (classExists_(year, cid)) return cid;

  // fallback: pilot class only
  const pilotCid = normalizeClassId_(pilot.classId);
  if (cid !== pilotCid) {
...
function classExists_(year, classId) {
  const y = norm_(year);
  const cid = normalizeClassId_(classId);
  if (!y || !cid) return false;
  const sh = ss_().getSheetByName(CFG.SHEET_CLASSES);
  if (!sh) return false;
  const values = sh.getDataRange().getValues();
  if (values.length < 2) return false;
  const idx = headerIndex_(values[0]);
  const iy = idx.year ?? idx.namhoc ?? 0;
  const ic = idx.classid ?? idx.classId ?? 1;
  for (let r = 1; r < values.length; r++) {
    const row = values[r];
    if (norm_(row[iy]) === y && normalizeClassId_(row[ic]) === cid) return true;
  }
  return false;
}
...
function logAi_(role, classId, studentId, action, model, tokens, note){
  try{
    appendRow_(CFG.SHEET_AI_LOGS, [
      nowIso_(),
      String(role||""),
      String(classId||""),
      String(studentId||""),
...
// ===========================
// STUDENT LOOKUP (NO MUTATION)
// ===========================
// ✅ Dùng khi đăng nhập HS/PH: KHÔNG được ghi đè "name" trong sheet.
function findStudent_(year, classId, studentId) {
  ...
  for (let i = 0; i < rows.length; i++) {
    const r = rows[i] || {};
    // chấp nhận cả dữ liệu thiếu year (đã có trong DB cũ)
    if (String(r.classId || "").trim() === cid && String(r.studentId || "").trim() === sid) {
      if (!y || !r.year || String(r.year).trim() === y) return r;
    }
  }
  return null;
}
...
function ensureSheets_() {
  ...
  const must = [
    ...
    CFG.SHEET_GROUPS,
    CFG.SHEET_GROUP_MEMBERS,
    CFG.SHEET_ASSIGNMENTS,
...
function listClassesPublic() {
  const pilot = getPilot_();
  // cache 60s giảm timeout
  try{
    const cache = CacheService.getScriptCache();
    const y0 = getActiveYear_() || "2025-2026";
    const ck = "listClassesPublic:" + y0 + ":" + String(pilot.lock||"");
  ...
  try{
    const cache = CacheService.getScriptCache();
    const ck = "listClassesPublic:" + y + ":" + String(pilot.lock||"");
    cache.put(ck, JSON.stringify({ classes: out }), 60);
  }catch(_){ }
...
function authStudentLogin({ year, classId, studentId }) {
  // ✅ Đăng nhập HS chỉ được XÁC THỰC, KHÔNG ghi đè dữ liệu hồ sơ.
  const cid = enforcePilotClass_(classId);
  const y = String(year || getActiveYear_() || "").trim();
  studentId = String(studentId || "").trim();
  if (!studentId) throw new Error("Thiếu mã học sinh");

  const st = lookupStudent_(y, cid, studentId);
  if (!st) throw new Error("Không tìm thấy học sinh: " + studentId);
  ...
  const session = createSession_({
    ...
    studentId,
    username: "",
    displayName: st.name || studentId || "Học sinh",
  });
  return { session, student: st };
}

function authParentLogin({ year, classId, studentId }) {
  // ✅ Đăng nhập PH: chỉ kiểm tra HS tồn tại, KHÔNG tạo mới/ghi đè.
  const cid = enforcePilotClass_(classId);
  const y = String(year || getActiveYear_() || "").trim();
  studentId = String(studentId || "").trim();
  if (!studentId) throw new Error("Thiếu mã học sinh");
  ...
  const session = createSession_({
    ...
    studentId,
    username: "",
    displayName: "PH " + (st.name || studentId),
  });
  return { session, student: st };
}
...
/**
 * lookupStudent_ (SSOT)
 * - Dùng cho đăng nhập HS/PH (lớp + mã HS)
 * - Trả về record HS hoặc null
 */
function lookupStudent_(year, classId, studentId) {
  const y = String(year || "").trim() || (getActiveYear_() || "2025-2026");
  const cid = normalizeClassId_(classId || "");
  const sid = String(studentId || "").trim();
  ...
  const rows = getAll_(CFG.SHEET_STUDENTS)
    .filter(r => String(r.year || "").trim() === y)
    .filter(r => normalizeClassId_(r.classId) === cid)
...
function adminCreateClass({ token, year, grade, classId, subject, textbook }) {
  ...
  // ✅ Admin Setup: luôn cho phép tạo thêm lớp (không bị chặn bởi pilot lock)
...
// AI: gợi ý cho học sinh (không làm hộ)
function aiStudentHint({ token, taskId, topic, body, draft, taskType, guardMode, signals, extraQuestion }) {

  const s = requireRoleSession_(token, ["STUDENT"]);
  const tTopic = String(topic || "").trim();
  const tBody  = String(body || "").trim();
  const dft    = String(draft || "").trim();

    const tType = String(taskType||"GENERAL").trim();
  const gMode = String(guardMode||"NORMAL").trim();
  const sig   = signals ? JSON.stringify(signals) : "{}";
  const qx    = String(extraQuestion||"").trim();

  const prompt =
`Bạn là trợ lý học tập cho học sinh tiểu học.
MỤC TIÊU: giúp học sinh TỰ LÀM, không làm hộ, không đưa đáp án hoàn chỉnh.

Ngữ cảnh:
- Dạng bài (taskType): ${tType}
- Chế độ an toàn (guardMode): ${gMode}
- Tiến bộ gần đây (signals): ${sig}
- Câu hỏi thêm của học sinh (nếu có): ${qx || "(không)"}

QUY TẮC:
- Nếu guardMode=NO_ANSWER: tuyệt đối không đưa đáp án; chỉ hướng dẫn 3–5 bước + gợi ý tự kiểm tra.
- Nếu học sinh chưa viết: đưa dàn ý/khung trả lời theo dạng bài.
- Nếu học sinh đã viết: chỉ góp ý 1–2 điểm + gợi ý bổ sung cụ thể.
- Trả lời tiếng Việt, thân thiện, ngắn gọn.

Bài tập:
- Mã bài: ${taskId||""}
- Chủ đề: ${String(topic||"").trim()}
- Nội dung: ${String(body||"").trim()}

Bài làm hiện tại:
${String(draft||"").trim() || "(chưa có)"}

Hãy đưa ra gợi ý:`.trim();

  const text = openaiText_(prompt, 220);

  // ✅ log AI usage
  const model = props_().getProperty("OPENAI_MODEL") || "gpt-4o-mini";
  logAi_(s.role, s.classId, s.studentId, "aiStudentHint", model, "", `taskId=${taskId||""}`);
...
/****************************************************
 * ✅ ADD-ON: GROUPS / ASSIGNMENTS / SUGGEST CARE
 * - Không phá luồng cũ
 * - Dùng SUBMISSIONS + FEEDBACKS hiện có để gợi ý nhóm
 ****************************************************/

function GA_norm_(v){ return String(v==null?"":v).trim(); }
function GA_upper_(v){ return GA_norm_(v).toUpperCase(); }
function GA_uuid_(prefix){ return (prefix||"X") + Utilities.getUuid().replace(/-/g,"").slice(0,12); }

function GA_ensureHeaders_(){
  ensureHeader_(CFG.SHEET_GROUPS, [
    "year","scopeClassId",
  ...
  ensureHeader_(CFG.SHEET_GROUP_MEMBERS, [
    "year","groupId","studentId",
    "addedAt","note"
  ]);

  ensureHeader_(CFG.SHEET_ASSIGNMENTS, [
    "year","scopeClassId","assignmentId",
    "taskId",
    "targetType","targetId",
    "dueAt","status",
    "createdBy","createdAt","updatedAt",
...
/** gọi trong ensureSheets_() nếu anh muốn chắc chắn header luôn tồn tại */
function GA_boot_(){
  try{ GA_ensureHeaders_(); }catch(_){}
}

/**
 * ===== GROUPS APIs =====
 * groupsList: lấy danh sách nhóm (kèm members)
 * groupsUpsert: tạo/sửa nhóm + cập nhật members
 * groupsDelete: xoá nhóm (hard delete theo groupId)
 */
function groupsList(body){
  const s = requireRoleSession_(body.token, ["TEACHER","ADMIN"]);
  const y = getActiveYear_() || "2025-2026";
  const scopeClassId = enforcePilotClass_(body.classId || s.classId || getPilot_().classId);

  GA_ensureHeaders_();

  const groups = getAll_(CFG.SHEET_GROUPS)
    .filter(r => GA_norm_(r.year)===y)
    .filter(r => GA_upper_(r.scopeClassId)===GA_upper_(scopeClassId))
    .map(r => ({
      year: y,
      scopeClassId,
      groupId: GA_norm_(r.groupId),
      groupName: GA_norm_(r.groupName),
      groupType: GA_norm_(r.groupType)||"CUSTOM",
      isSystem: GA_norm_(r.isSystem)==="1",
      ruleJson: GA_norm_(r.ruleJson||""),
      createdBy: GA_norm_(r.createdBy||""),
      createdAt: GA_norm_(r.createdAt||""),
      updatedAt: GA_norm_(r.updatedAt||""),
    }))
    .filter(x=>x.groupId);

  const members = getAll_(CFG.SHEET_GROUP_MEMBERS)
    .filter(r => GA_norm_(r.year)===y)
    .map(r => ({
      groupId: GA_norm_(r.groupId),
      studentId: GA_norm_(r.studentId),
      addedAt: GA_norm_(r.addedAt||""),
      note: GA_norm_(r.note||"")
    }))
    .filter(x=>x.groupId && x.studentId);

  const memMap = {};
  members.forEach(m=>{
    memMap[m.groupId] = memMap[m.groupId] || [];
    memMap[m.groupId].push(m);
  });

  groups.forEach(g=> g.members = memMap[g.groupId] || []);

  groups.sort((a,b)=> String(a.groupName).localeCompare(String(b.groupName)));
  return { year:y, classId:scopeClassId, groups };
}

function groupsUpsert(body){
  const s = requireRoleSession_(body.token, ["TEACHER","ADMIN"]);
  const y = getActiveYear_() || "2025-2026";
  const scopeClassId = enforcePilotClass_(body.classId || s.classId || getPilot_().classId);
  ...
  let groupId = GA_norm_(group.groupId);
  const groupName = GA_norm_(group.groupName);
  const groupType = GA_norm_(group.groupType) || "CUSTOM";
  const isSystem = (String(group.isSystem||"") === "1" || group.isSystem===true) ? "1" : "0";
  let ruleJson = "";
    if (group.ruleJson != null && group.ruleJson !== "") {
      if (typeof group.ruleJson === "string") ruleJson = group.ruleJson;
      else ruleJson = JSON.stringify(group.ruleJson);
    }


  if(!groupName) throw new Error("Thiếu groupName");
  if(!groupId) groupId = GA_uuid_("G");

  const sh = sh_(CFG.SHEET_GROUPS);
  const data = sh.getDataRange().getValues();
  const header = data[0].map(x=>String(x||"").trim());
  const idx = {}; header.forEach((h,i)=>idx[h]=i);
  ...
  for(let i=1;i<data.length;i++){
    const r=data[i];
    if(GA_norm_(r[idx.year])===y &&
       GA_upper_(r[idx.scopeClassId])===GA_upper_(scopeClassId) &&
       GA_norm_(r[idx.groupId])===groupId){
      rowFound = i+1; break;
    }
  }

  const createdAt = (rowFound>0)
    ? GA_norm_(sh.getRange(rowFound, idx.createdAt+1).getValue()||now)
    : now;
  ...
  // ✅ FIX: nếu client gửi field members (kể cả rỗng) → replace toàn bộ
  const hasMembersField = Object.prototype.hasOwnProperty.call(group, "members");
  const members = Array.isArray(group.members) ? group.members : [];

  if (hasMembersField) {
    const msh = sh_(CFG.SHEET_GROUP_MEMBERS);
    const mdata = msh.getDataRange().getValues();
    const mh = mdata[0].map(x=>String(x||"").trim());
    const midx = {}; mh.forEach((h,i)=>midx[h]=i);

    // xoá members cũ
    const delRows = [];
    for(let i=1;i<mdata.length;i++){
      const r=mdata[i];
      if(GA_norm_(r[midx.year])===y && GA_norm_(r[midx.groupId])===groupId){
        delRows.push(i+1);
      }
    }
    delRows.reverse().forEach(rr=>msh.deleteRow(rr));

    // thêm members mới (có thể = 0)
    members.forEach(m=>{
      const sid = GA_norm_(m.studentId || m);
      if(!sid) return;
      msh.appendRow([y, groupId, sid, nowIso_(), GA_norm_(m.note||"")]);
    });
  }

  return { saved:true, year:y, classId:scopeClassId, groupId };
}

function groupsDelete(body){
  const s = requireRoleSession_(body.token, ["TEACHER","ADMIN"]);
  const y = getActiveYear_() || "2025-2026";
  const scopeClassId = enforcePilotClass_(body.classId || s.classId || getPilot_().classId);
  const groupId = GA_norm_(body.groupId);
  ...
  return { deleted:true, year:y, classId:scopeClassId, groupId };
}

/**
 * ===== ASSIGNMENTS APIs =====
 * assignmentsCreate: giao 1 task cho CLASS/GROUP/STUDENT
 * assignmentsList: liệt kê giao bài theo class scope
 * assignmentsGet: chi tiết
 */
function assignmentsCreate(body){
  const s = requireRoleSession_(body.token, ["TEACHER","ADMIN"]);
  const y = getActiveYear_() || "2025-2026";
  const scopeClassId = enforcePilotClass_(body.classId || s.classId || getPilot_().classId);
  ...
  const taskId = GA_norm_(body.taskId);
  const targetType = GA_upper_(body.targetType); // CLASS | GROUP | STUDENT
  const targetId = GA_norm_(body.targetId);
  const dueAt = GA_norm_(body.dueAt||"");
  const note = GA_norm_(body.note||"");

  if(!taskId) throw new Error("Thiếu taskId");
  if(!["CLASS","GROUP","STUDENT"].includes(targetType))
    throw new Error("targetType phải là CLASS/GROUP/STUDENT");
  if(!targetId) throw new Error("Thiếu targetId");

  const assignmentId = GA_uuid_("A");
  const now = nowIso_();

  appendRow_(CFG.SHEET_ASSIGNMENTS, [
    y,
    scopeClassId,          // ✅ FIX: gắn scope lớp
    assignmentId,
    taskId,
    targetType,
    targetId,
  ...
  return { created:true, year:y, classId:scopeClassId, assignmentId };
}

function assignmentsList(body){
  const s = requireRoleSession_(body.token, ["TEACHER","ADMIN"]);
  const y = getActiveYear_() || "2025-2026";
  const scopeClassId = enforcePilotClass_(body.classId || s.classId || getPilot_().classId);
  ...
  let rows = getAll_(CFG.SHEET_ASSIGNMENTS)
    .filter(r => GA_norm_(r.year) === y)
    .filter(r => GA_upper_(r.scopeClassId) === GA_upper_(scopeClassId))
    .map(r => ({
      year: y,
      assignmentId: GA_norm_(r.assignmentId),
      taskId: GA_norm_(r.taskId),
      targetType: GA_upper_(r.targetType),
      targetId: GA_norm_(r.targetId),
      dueAt: GA_norm_(r.dueAt||""),
      status: GA_upper_(r.status||"ACTIVE"),
      createdBy: GA_norm_(r.createdBy||""),
      createdAt: GA_norm_(r.createdAt||""),
      updatedAt: GA_norm_(r.updatedAt||""),
      note: GA_norm_(r.note||""),
  ...
  // filter optional
  const ft = GA_upper_(body.targetType||"");
  const fid = GA_norm_(body.targetId||"");
  const ftask = GA_norm_(body.taskId||"");

  if(ft) rows = rows.filter(x=>x.targetType===ft);
  if(fid) rows = rows.filter(x=>x.targetId===fid);
  if(ftask) rows = rows.filter(x=>x.taskId===ftask);

  rows.sort((a,b)=> String(b.createdAt).localeCompare(String(a.createdAt)));
  return { year:y, classId:scopeClassId, assignments: rows };
}

function assignmentsGet(body){
  const s = requireRoleSession_(body.token, ["TEACHER","ADMIN"]);
  const y = getActiveYear_() || "2025-2026";
  enforcePilotClass_(body.classId || s.classId || getPilot_().classId);

  GA_ensureHeaders_();

  const assignmentId = GA_norm_(body.assignmentId);
  if(!assignmentId) throw new Error("Thiếu assignmentId");

  const row = getAll_(CFG.SHEET_ASSIGNMENTS)
    .find(r => GA_norm_(r.year)===y && GA_norm_(r.assignmentId)===assignmentId);

  if(!row) throw new Error("Không tìm thấy assignment");
  return { year:y, assignment: row };
}

/**
 * ===== SMART SUGGEST GROUPS =====
 * groupsSuggestCare:
 * - 7 ngày chưa nộp => "Tăng tốc"
 * - điểm < 5 liên tiếp => "Cần hỗ trợ"
 * - tiến bộ nhanh => "Nâng cao"
 */
function groupsSuggestCare(body){
  const s = requireRoleSession_(body.token, ["TEACHER","ADMIN"]);
  const y = getActiveYear_() || "2025-2026";
  const scopeClassId = enforcePilotClass_(body.classId || s.classId || getPilot_().classId);

  const daysNoSubmit = Number(body.daysNoSubmit || 7);
  const lowScoreThreshold = Number(body.lowScoreThreshold || 5);
  const lowScoreStreak = Number(body.lowScoreStreak || 2);
  const fastImproveMin = Number(body.fastImproveMin || 7);

  // students
  const students = getAll_(CFG.SHEET_STUDENTS)
    .filter(r => GA_norm_(r.year)===y)
    .filter(r => GA_upper_(r.classId)===GA_upper_(scopeClassId))
    .map(r => ({ studentId: GA_norm_(r.studentId), name: GA_norm_(r.name||r.studentId) }))
    .filter(x=>x.studentId);
  ...
  // last submit per student
  const lastSubmit = {};
  subs.forEach(r=>{
    const sid = GA_norm_(r.studentId);
    const dt = new Date(r.createdAt);
    if(!sid || isNaN(dt)) return;
    if(!lastSubmit[sid] || dt > lastSubmit[sid]) lastSubmit[sid] = dt;
  });

  // feedback list per student (sorted desc)
  const fbMap = {};
  fbs.forEach(r=>{
    const sid = GA_norm_(r.studentId);
    if(!sid) return;
    ...
    const mx = Number(r.scoreMax);
    let norm10 = NaN;
    if(!isNaN(sc)){
      if(!isNaN(mx) && mx>0) norm10 = (sc/mx)*10;
      else norm10 = sc; // assume already 0..10
    }
    fbMap[sid] = fbMap[sid] || [];
    fbMap[sid].push({ dt, norm10 });
  });
  Object.keys(fbMap).forEach(sid=>{
    fbMap[sid].sort((a,b)=> b.dt - a.dt);
  });

  const now = new Date();
  const cutoff = new Date(now.getTime() - daysNoSubmit*24*3600*1000);
  ...
  students.forEach(st=>{
    ...
    // rule 1: no submit
    const ls = lastSubmit[sid];
    if(!ls || ls < cutoff){
      tangToc.push(st);
    }

    // rule 2: low score streak
    const fl = fbMap[sid] || [];
    if(fl.length >= lowScoreStreak){
      const recent = fl.slice(0, lowScoreStreak).map(x=>x.norm10).filter(x=>!isNaN(x));
      if(recent.length === lowScoreStreak && recent.every(x=>x < lowScoreThreshold)){
        canHoTro.push(st);
      }
    }

    // rule 3: improve fast (3 recent increasing and last >= fastImproveMin)
    if(fl.length >= 3){
      const a = fl[2]?.norm10, b = fl[1]?.norm10, c = fl[0]?.norm10;
      if(!isNaN(a) && !isNaN(b) && !isNaN(c) && a < b && b < c && c >= fastImproveMin){
        nangCao.push(st);
      }
    }
  });
  ...
  return {
    ...
    suggested: {
      tangToc: { groupName:"Tăng tốc", reason:`${daysNoSubmit} ngày chưa nộp`, students: uniq(tangToc) },
      canHoTro: { groupName:"Cần hỗ trợ", reason:`điểm < ${lowScoreThreshold} liên tiếp ${lowScoreStreak} lần`, students: uniq(canHoTro) },
      nangCao: { groupName:"Nâng cao", reason:`tiến bộ nhanh (3 lần tăng dần, gần nhất ≥ ${fastImproveMin})`, students: uniq(nangCao) },
    }
  };
}
...
/**
 * ✅ teacherTargets (SSOT)
 * Trả 1 payload duy nhất cho teacher-task.html:
 * - classes: lớp theo năm active
 * - groups: nhóm theo classId (kèm memberCount)
 * - students: danh sách HS theo classId
 */
function teacherTargets(body){
  const s = requireRoleSession_(body.token, ["TEACHER","ADMIN"]);
  const pilot = getPilot_();
  const y = getActiveYear_() || "2025-2026";

  // classId có thể truyền hoặc lấy theo session/pilot
  const cid = enforcePilotClass_(body.classId || s.classId || pilot.classId);

  // đảm bảo headers nhóm/giao bài (nếu đã thêm add-on groups)
  try{ if (typeof GA_ensureHeaders_==="function") GA_ensureHeaders_(); }catch(_){}

  // 1) classes
  let classes = [];
  try {
    // dùng logic hiện có để chuẩn nhất
    const r = listClassesForTeacher({ token: body.token });
    classes = (r && r.classes) ? r.classes : [];
  } catch (e) {
    // fallback đọc sheet
    classes = getAll_(CFG.SHEET_CLASSES)
      .filter(r => String(r.year||"").trim()===y)
      .map(r => ({
        classId: String(r.classId||"").trim(),
        className: String(r.className||r.classId||"").trim(),
  ...
  // 2) students (theo class đang chọn)
  const students = getAll_(CFG.SHEET_STUDENTS)
    .filter(r => String(r.year||"").trim()===y)
    .filter(r => normalizeClassId_(r.classId)===normalizeClassId_(cid))
    .map(r => ({
  ...
  // 3) groups (theo class đang chọn)
  let groups = [];
  if (CFG.SHEET_GROUPS && ss_().getSheetByName(CFG.SHEET_GROUPS)) {
    const gr = getAll_(CFG.SHEET_GROUPS)
      .filter(r => String(r.year||"").trim()===y)
      .filter(r => normalizeClassId_(r.scopeClassId)===normalizeClassId_(cid))
      .map(r => ({
        groupId: String(r.groupId||"").trim(),
        groupName: String(r.groupName||"").trim(),
        groupType: String(r.groupType||"CUSTOM").trim(),
        isSystem: String(r.isSystem||"") === "1",
    ...
    // memberCount
    let memberCount = {};
    if (CFG.SHEET_GROUP_MEMBERS && ss_().getSheetByName(CFG.SHEET_GROUP_MEMBERS)) {
      const mem = getAll_(CFG.SHEET_GROUP_MEMBERS)
        .filter(r => String(r.year||"").trim()===y);
      mem.forEach(m=>{
    ...
    groups = gr.map(g => ({ ...g, memberCount: memberCount[g.groupId] || 0 }))
              .sort((a,b)=> String(a.groupName).localeCompare(String(b.groupName)));
  }

  // 4) targets gợi ý để UI dựng dropdown 1 phát
  const targets = {
    classDefault: { targetType: "CLASS", targetId: cid, label: `Cả lớp ${cid}` },
    groups: groups.map(g => ({
      targetType: "GROUP",
      targetId: g.groupId,
      label: `${g.groupName} (${g.memberCount||0})`,
      meta: g
    })),
    students: students.map(st => ({
      targetType: "STUDENT",
      targetId: st.studentId,
      label: `${st.studentId} – ${st.name}`,
      meta: st
    }))
  };
